...
def main():
    ...
    cmd = "/root/miniconda3/bin/python -u eval.py --network=resnet101 --method=baseline --batch-size=4 --data-dir ./dataset/cityscapes --data-list ./dataset/list/cityscapes/val.lst --dataset cityscapes_train --store-output=True --input-size 256,512 --use-flip=False --use-ms=False --gpu 0 --predict-choice whole --whole-scale 1 &> val_output"
	...
	if iter % 1000 == 0:
	    torch.save(deeplab.state_dict(),osp.join(args.snapshot_dir, 'CS_scenes_'+str(args.num_steps)+'.pth')
            cmd = cmd + "--restore-from" + osp.join(args.snapshot_dir, 'CS_scenes_'+str(args.num_steps)+'.pth')  
            subprocess.call(cmd)
            lines = []
            temp = 0
            with open("val_output", "rt") as f:
                for line in f:
                   lines.append(line)
                temp = float(str.join(mylines[3058][11:25]))
            if temp <= val_IoU:
                val_IoU = temp
            else:
